...
class DamidActivationDistanceStep(Step):
    ...
    def task(batch_id, cfg, tmp_dir):
        ...
        logger.info(cfg.get('restraints/DamID/contact_range'))
...
def get_damid_actdist(locid, pwish, plast, hss, it_corr, contact_range=0.05, shape="sphere", nucleus_param=5000.0):
    """
    ...
    ----------
        ...
        pwish : float
            target contact probability (from input DamID file, upon preprocessing of raw data)
        plast : float
            the last refined probability (using pwish and the iterative refinement)
        hss : alabtools.analysis.HssFile
            file containing coordinates
        contact_range : float
            contact range of sum of radius of beads
        it_corr: boolean
            if 1, then we correct pwish using the iterative refinement
        shape : str
